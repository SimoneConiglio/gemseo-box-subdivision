...
points are a **sweep** and not a batch. This module does the same for the
convexity setting, patching
:meth:`.OuterApproximationOptimizer._solve_milp` the way
`benchmarks/trust_region.py` patches the master problem:
...
to span and the stub gives it the top rung, the conservative end, rather than
leaving a run with its cuts unguarded.

:::{note}
The policy itself is in :mod:`gemseo_box_subdivision.convexity_sweep`, free of
GEMSEO, so that it can be tested on its own and moved into
`gemseo-bilevel-outer-approximation`, where the iteration loop lives. What is
stubbed here is only the wiring to the released master.
:::
...
from gemseo_box_subdivision.convexity_sweep import Deployment
...
def _probe_index(optimizer: Any, current_step: float | None, n_rungs: int) -> int:
    """Return the rung a call of the master starts from.

    The master does not say which of its parallel probes is calling: it says
    which trust-region radius the probe was given, out of the
    ``geomspace(step / 2, step)`` it spreads them over. Recovering the index
    from the radius pairs the two ladders, which is the pairing that makes
    sense: the tight region and the raw cuts both exploit, the wide region and
    the dominated cuts both explore.

    Args:
        optimizer: The master.
        current_step: The radius the probe was given, if any.
        n_rungs: The number of rungs of the convexity ladder.

    Returns:
        The index of the rung. A master with nowhere to spread its probes gets
        the **top** rung, the conservative end of the ladder: a single probe at
        the bottom is a run with its cuts unguarded, which converges after two or
        three boxes and reports success far from the optimum. That is also the
        rung :meth:`.BoxSubdivisionSettings.to_master_settings` falls back to.
        A call made outside the probing loop passes the radius of the master
        itself, which is the top of the radius ladder, so it lands there too.
    """
    ...
    probe = int(argmin(abs(steps - current_step)))
    return min(probe * n_rungs // n_points, n_rungs - 1)
...
def parallel_convexity_sweep(
    ...
    def patched(self, *args, **kwargs):  # noqa: ANN001, ANN002, ANN003, ANN202
        ...
        index = _probe_index(self, current_step, sweep.n_points)
        saved = getattr(self, setting_name)
        result = None
        try:
            ...
            setattr(self, setting_name, saved)
...
def run(problem, seed, sweep, margin):  # noqa: ANN001, ANN201
    ...
    Args:
        ...
        margin: The fixed margin, which the sweep replaces.

    Returns:
        The outcome of the run, and the deployments of the sweep.
    """
    overrides = {"min_dfk": margin, "max_step": TRUST_REGION_RADIUS}
    if sweep is None:
    ...
    with parallel_convexity_sweep(sweep) as trace:
...
SETUPS = (
    ("fixed, margin 1", None, 1.0),
    ("fixed, margin 10", None, 10.0),
    ("fixed, margin 100", None, 100.0),
    ("sweep, max 100", ConvexitySweepSettings(max_value=100.0), 100.0),
    ("sweep, max 1000", ConvexitySweepSettings(max_value=1000.0), 100.0),
    (
        "sweep, observed",
        ConvexitySweepSettings(headroom=1.0),
        100.0,
    ),
    (
        "sweep, observed x 10",
        ConvexitySweepSettings(headroom=10.0),
        100.0,
    ),
)
...
def main() -> None:
    ...
    print(
        ...
        f"over {len(SEEDS)} starting points\n"
    )
    print(
    ...
    for name in PROBLEM_NAMES:
        ...
        for label, sweep, margin in SETUPS:
            gaps = []
            costs = []
            escalated = 0
            deployed = 0
            for seed in SEEDS:
                with suppress(Exception):
                    outcome, trace = run(benchmark, seed, sweep, margin)
